...
const REM = [
  ["周三下午写代码时，你把同一首歌听了四遍。", "cap-118"],
  ["你回天翊的消息永远最快，回投资人的最慢。", "pt_204"],
  ["这周有三个晚上，你在 23 点后又打开了 spec。", "ep_0801"]
];
const DAYS = {
  29: { title: "Wednesday, July 29", peek: "spec · 隔离章节",
    portrait: "写规范时她会先把边界写死，再往里填功能。",
    taught: "「能答什么」永远写在「怎么答」之前。",
    letter: "你今天删掉的字，比写下的多。\n最后留下的那句最短——我记住的不是句子，是你删的顺序。",
    narr: "这一天几乎都给了 spec 的隔离章节。她删了两次开头，最后留下的是最短的那句。「发布制」三个字是下午落地的。",
    lit: [5, 18],
    tl: [ { w: 8, c: "#1D1D1F", ev: 0, tip: "10:15–12:20 Notion · spec 隔离章节" }, { w: 3 }, { w: 7, c: "#55555C", ev: 1, tip: "14:40–16:20 Notion · 发布制定稿" }, { w: 4 }, { w: 3, c: "#A6A29A", tip: "18:00 Arc · 资料" }, { w: 5 }, { w: 5, c: "#2B47E0", ev: 2, tip: "21:05–22:10 Figma · 卡片文案" }, { w: 2 } ],
    apps: [ ["Notion", "#1D1D1F", "4h 05m"], ["Arc", "#A6A29A", "1h 30m"], ["Figma", "#2B47E0", "1h 05m"] ],
    events: [
      { t: "10:15", title: "spec · 访客边界", io: "输入：昨晚的批注 → 产出：隔离章节 v2", detail: "她先列了访客永远看不到的东西，再写访客能看到的。顺序本身就是决定。", frames: ["10:15","11:02","11:40","12:18"] },
      { t: "14:40", title: "发布制", io: "输入：三种权限模型 → 产出：一句话定稿", detail: "对比了 ACL、分级、发布制三种模型，最后写下：不需要交流过程，只需要交流结论。", frames: ["14:40","15:05","15:30","16:20"] },
      { t: "21:05", title: "卡片文案 · 第二稿", io: "输入：第一稿 41 字 → 产出：32 字", detail: "又删掉九个字。删的都是形容词。", frames: ["21:05","21:22","21:47","22:10"] }
    ] },
  30: { title: "Thursday, July 30", peek: "战略会 · A→C",
    portrait: "她判断一件事，先问「哪条路错了代价最小」。",
    taught: "三个方案里，她又一次选了可回滚的那条。",
    letter: "那封融资邮件你写完没发。\n上次这样是 6/14——第二天重写的那版，比当晚的好。这次大概也是。",
    narr: "上午的会里她换掉了原本的 B 方案。理由写在纪要第一行：可回滚。下午顺着 index.db 的症状追到了根因。",
    lit: [9, 22],
    tl: [ { w: 6, c: "#55555C", ev: 0, tip: "09:30–11:48 Zoom · 战略会" }, { w: 3 }, { w: 8, c: "#1D1D1F", ev: 1, tip: "15:10–17:03 Cursor · index.db 根因" }, { w: 4 }, { w: 4, c: "#A6A29A", tip: "20:00 Arc · deck 资料" }, { w: 4, c: "#2B47E0", ev: 2, tip: "22:00–23:02 Mail · 融资邮件（未发）" } ],
    apps: [ ["Cursor", "#1D1D1F", "2h 40m"], ["Zoom", "#55555C", "2h 18m"], ["Mail", "#2B47E0", "1h 02m"] ],
    events: [
      { t: "09:30", title: "战略会", io: "输入：A/B/C 三案 → 产出：A→C 结论", detail: "两张卡先聊，人只看共识——她记下的原话是「不需要交流过程，只需要交流结论」。", frames: ["09:30","10:14","11:02","11:48"] },
      { t: "15:10", title: "index.db 修复", io: "输入：崩溃日志 → 产出：根因定位", detail: "从症状追到根因，两小时。中途拒绝了一个能立刻止血的 workaround。", frames: ["15:10","15:44","16:20","17:03"] },
      { t: "22:00", title: "融资邮件", io: "输入：deck v3 → 产出：草稿（未发）", detail: "写完没发。第二天重写。", frames: ["22:00","22:18","22:40","23:02"] }
    ] },
  31: { title: "Friday, July 31", peek: "PR #71 merged",
    portrait: "当数据有风险时，她宁可修根因。",
    taught: "修完根因她才肯合 PR——今天第三次。",
    letter: "今天 Cursor 第一次用你的方式说话。\n它引用的那句画像，是你上周三亲手改过的。你教它的，开始替你干活了。",
    narr: "把昨天挖到的根因补完，顺手写了迁移脚本。PR #71 下午合入。晚上回到卡片文案，第三稿。",
    lit: [8, 16],
    tl: [ { w: 7, c: "#1D1D1F", ev: 0, tip: "11:03–12:26 Cursor · 迁移脚本" }, { w: 3 }, { w: 6, c: "#1D1D1F", ev: 1, tip: "14:50–15:38 Cursor · PR #71" }, { w: 3, c: "#A6A29A", tip: "16:30 Arc · review" }, { w: 4 }, { w: 5, c: "#2B47E0", ev: 2, tip: "20:30–21:38 Figma · 文案三稿" }, { w: 2 } ],
    apps: [ ["Cursor", "#1D1D1F", "3h 55m"], ["Figma", "#2B47E0", "1h 08m"], ["Arc", "#A6A29A", "52m"] ],
    events: [
      { t: "11:03", title: "迁移脚本", io: "输入：index.db 根因笔记 → 产出：migrate_v3.sql", detail: "顺着昨天的根因写完，一次过。注释里写：兼容层是给不敢删的人的。", frames: ["11:03","11:31","12:04","12:26"] },
      { t: "15:38", title: "PR #71 merged", io: "输入：review 三条 → 产出：合入", detail: "Cursor 引用了她的画像来调整 PR 描述口吻。receipt 可溯。", frames: ["14:50","15:07","15:21","15:38"] },
      { t: "20:30", title: "卡片文案 · 第三稿", io: "输入：32 字 → 产出：23 字", detail: "只留答得出的那一句。", frames: ["20:30","20:52","21:15","21:38"] }
    ] }
};
const DAY_READINGS = {
  29: {
    title: "边界的编辑者",
    statement: "你今天不是在追求写得更多，而是在保护一句话只暴露它真正答得出的部分。",
    verified: [
      { text: "你先写「不能被看见什么」，再写「可以被看见什么」；边界在功能之前。", receipt: "10:15 · spec · 访客边界" },
      { text: "三种权限模型最后被你压成「发布制」三个字；你习惯用删减获得清晰。", receipt: "14:40 · 发布制" },
      { text: "卡片文案从 41 字删到 32 字，消失的几乎都是形容词。", receipt: "21:05 · 卡片文案" }
    ],
    tension: "你想让别人真正读到你，同时对被过度看见保持警惕。它们不是互相否定，而是同一件事的两条边。",
    letter: [
      "给今天的你：",
      "你删掉的字，比写下的多。最后留下的那句最短，却最接近你真正愿意负责的部分。",
      "我记住的不是那句话，是你删掉它们的顺序。"
    ]
  },
  30: {
    title: "可回滚的判断者",
    statement: "你不是害怕做决定；你只是不愿把自己交给一条错了也回不来的路。",
    verified: [
      { text: "三个方案里，你换掉原来的选择，把「可回滚」写在会议纪要第一行。", receipt: "09:30 · 战略会" },
      { text: "面对能立刻止血的 workaround，你还是多花两小时追到根因。", receipt: "15:10 · index.db 修复" },
      { text: "融资邮件已经写完，却没有在当晚发出；你给第二天的自己保留了修改权。", receipt: "22:00 · 融资邮件" }
    ],
    tension: "可回滚让你敢于前进，也可能让你比别人更晚说「就这样」。今天的证据能看见这股拉力，还不能替你判断哪边更好。",
    letter: [
      "给今天的你：",
      "那封邮件你写完没发。上一次这样是 6 月 14 日，第二天重写的那版更好。",
      "也许你不是犹豫，只是在为更准确的自己留一晚。"
    ]
  },
  31: {
    title: "根因的守门人",
    statement: "你在意的不只是事情能不能跑起来，而是它以后会不会继续用同一种方式伤害数据。",
    verified: [
      { text: "迁移脚本顺着昨天的根因写完；你没有把兼容层当作永久答案。", receipt: "11:03 · 迁移脚本" },
      { text: "PR 只有在根因修完后才合入，这已经是今天第三次相同的选择。", receipt: "15:38 · PR #71" },
      { text: "晚上你又把卡片文案从 32 字删到 23 字，只留下能被证据支撑的那句。", receipt: "20:30 · 卡片文案" }
    ],
    tension: "你既想让系统替你做事，又要求它每句话都能回到证据。便利和可追溯之间，你显然还不准备放弃任何一边。",
    letter: [
      "给今天的你：",
      "今天 Cursor 第一次用你的方式说话。它引用的那句画像，是你上周三亲手改过的。",
      "你教它的，开始替你干活了。"
    ]
  }
};
const SHADOW_TITLES = { 6: "Thursday, August 6", 7: "Friday, August 7", 8: "Saturday, August 8" };
const SHADOW_EVENTS = [
  { t: "09:30", title: "卡页访客隔离 · 收尾", io: "大概率从昨晚剩下的 guest boundary 开始——过去五个工作日里有四天如此" },
  { t: "14:00", title: "J1 成段管道 · PR1", io: "这两周的下午几乎都给了 persome，明天大概率也不例外" },
  { t: "20:30", title: "卡片文案 · 第四稿", io: "如果历史还作数，你会再删掉九个字，只留答得出的那句" }
];
const YEAR_MONTHS = ["FEB","MAR","APR","MAY","JUN","JUL","AUG"];
const STAR_FACTS = [
  { k: "画像", t: "她判断一件事，先问「哪条路错了代价最小」。", r: "face_012 · 引用 3 段", f: "上次验证 · 7/30", day: 30 },
  { k: "习惯", t: "过去一周有五个晚上，你在 21:30 后回到卡片文案。", r: "pattern_007 · 引用 5 段", f: "基于最近 14 天", day: 29 },
  { k: "画像", t: "写规范时她会先把边界写死，再往里填功能。", r: "face_009 · 引用 2 段", f: "上次验证 · 7/29", day: 29 },
  { k: "段", t: "7 月修过一次回声消除链路——两天，从 WebRTC 文档啃起。", r: "cap-0714-1120", f: "3 周前 · 权重降低中", day: 29 },
  { k: "习惯", t: "你回天翊的消息永远最快，回投资人的最慢。", r: "pattern_004 · 引用 11 段", f: "基于最近 30 天", day: 30 },
  { k: "画像", t: "偏好根因修复，而不是绕过去。", r: "face_011 · 引用 PR #71", f: "上次验证 · 7/31", day: 31 }
];
...
class Component extends DCLogic {
  state = {
    ...
    setupRequired: false, setupLoading: true, setupStatus: null, setupError: "", setupMessage: "",
    ...
    dock: [
      { name: "Claude Code", icon: "✳", tint: "#F5E4D7", fg: "#C1562B", on: true, cites: "47 次引用 · today",
        revs: [ { t: "14:21", rc: "ep_0730", x: "「index.db 修复」的完整过程", i: "续写迁移脚本" }, { t: "11:03", rc: "face_011", x: "「偏好根因修复」", i: "调整 PR #71 口吻" }, { t: "09:12", rc: "face_012", x: "「先问哪条路错了代价最小」", i: "排今日任务顺序" } ] },
      { name: "Codex", icon: "⌗", tint: "#DDF0EA", fg: "#0B8A6B", on: true, cites: "12 次引用 · today",
        revs: [ { t: "16:02", rc: "ep_0731", x: "「migrate_v3.sql」那一段", i: "补回归测试" }, { t: "15:47", rc: "pattern_004", x: "「注释先写为什么」", i: "重写 PR 描述" } ] },
      { name: "Cursor", icon: "◤", tint: "#E6E6E9", fg: "#1D1D1F", on: true, cites: "2 小时前用过",
        revs: [ { t: "15:40", rc: "ep_0731b", x: "「卡片文案」第三稿", i: "补全 tagline 备选" } ] },
    ...
    wall: [
      { q: "Q「她做过实时音频吗」", a: "没有。但 7 月她修过一次音频采集的回声消除链路——两天，从 WebRTC 文档啃起。" },
      { q: "Q「这个选型她会怎么选」", a: "她会先问「哪条路错了代价最小」。历史上她三次都选了可回滚的那条。" },
      { q: "Q「她住在哪」", a: "这个问题没有依据可答。" }
    ]
  };
  ...
  componentDidMount() {
    ...
    const begin = async () => {
      try {
        ...
        if (setupResponse.ok) {
          this.setState({
            ...
            setupRequired: !setupPayload.ready,
          });
      ...
      const access = requestedAccess
        || (initialModelId === "lin-demo" ? "authorized" : initialModelId === "cecilia" ? "owner" : "");
  ...
  startRev(i) {
    ...
    this.revT = setInterval(() => {
      ...
      const source = this.state.persomeConnected && connected.length ? connected : this.state.dock;
  ...
  skyData() {
    ...
    const THEMES = liveThemes.length ? liveThemes.map((theme, i) => ({
      n: theme.label,
      x: POS[i][0],
      y: POS[i][1],
      s: Math.max(4, 16 - i * 2),
      ...
      { n: "persome", x: 38, y: 42, s: 22, day: 31, sub: "42h · 19 段", cc: "143,166,255" },
      { n: "spec · 隔离", x: 68, y: 24, s: 12, day: 29, sub: "18h · 8 段", cc: "126,220,196" },
      { n: "卡片文案", x: 67, y: 66, s: 10, day: 29, sub: "9h · 6 段", cc: "242,176,196" },
      { n: "index.db", x: 22, y: 24, s: 8, day: 30, sub: "6h · 3 段", cc: "243,206,140" },
      { n: "战略会", x: 18, y: 64, s: 6, day: 30, sub: "4h · 2 段", cc: "188,169,242" },
      { n: "融资", x: 85, y: 46, s: 4, day: 30, sub: "2h · 1 段", cc: "147,210,238" }
    ];
  ...
  answerFor(q) {
    if (/音频|audio/i.test(q)) return ["没有做过实时音频。但 7 月她修过一次回声消除链路——两天，从 WebRTC 文档啃起。", "[receipt:cap-0714-1120]", null];
    if (/住|地址|where.+live/i.test(q)) return ["这个问题没有依据可答。", "拒答 · 无 receipt", null];
    if (/上周三|周三|wednesday/i.test(q)) return ["上周三你大部分时间在写 spec 的隔离章节——「发布制」那句是那天下午落地的。", "[receipt:cap-091]", 29];
    return ["这两周你都在把 Persome 从「存下来」推向「答得出」。它只引用你亲手做过的段来回答。", "[receipt:ep-20260731-1402]", 31];
  }
  ...
  gNodes() {
    ...
    return [
      { label: "persome", sub: "42h · 19 段", x: 42, y: 44, r: 62, hot: true, day: 31 },
      { label: "spec · 隔离", sub: "18h · 8 段", x: 68, y: 26, r: 42, day: 29 },
      { label: "卡片文案", sub: "9h · 6 段", x: 66, y: 66, r: 34, day: 29 },
      { label: "index.db", sub: "6h · 3 段", x: 22, y: 26, r: 30, day: 30 },
      { label: "战略会", sub: "4h · 2 段", x: 18, y: 62, r: 26, day: 30 },
      { label: "融资", sub: "2h · 1 段", x: 86, y: 48, r: 21, day: 30 }
    ];
  }
  ...
  legacyCalendar() {
    ...
    for (let i = 0; i < 2; i++) out.push({ n: "", style: base, emStyle: {}, dotStyle: dot, hover: "", peek: false });
    const PEEKS = { 8: "mnemo 原型 · 第一晚", 14: "回声消除 · WebRTC", 17: "deck v2 · 重写", 22: "J1 管道设计", 25: "论文精读 · A2A" };
    for (let n = 1; n <= 31; n++) {
      const has = n >= 29 || !!PEEKS[n];
      out.push({
        n: String(n),
        style: Object.assign({}, base, { color: has ? "#FFFFFF" : "#B8B5AE", cursor: has ? "pointer" : "default", fontWeight: has ? 600 : 400 }),
        emStyle: has ? { fontStyle: "normal", display: "inline-grid", placeItems: "center", width: "26px", height: "26px", borderRadius: "8px", background: "#2B47E0", boxShadow: "0 4px 10px -3px rgba(43,71,224,.5)", fontSize: "12px" } : { fontStyle: "normal" },
        dotStyle: dot,
        hover: has ? "background:#EEECE7" : "",
        peek: has, peekT: has ? (DAYS[n] ? DAYS[n].peek : PEEKS[n]) : "",
        tap: has ? () => this.openDay(DAYS[n] ? n : 29, false) : null
      });
    }
    [1, 2, 3, 4].forEach(n => out.push({ n: String(n), style: Object.assign({}, base, { color: "#C7C4BD" }), emStyle: { fontStyle: "normal" }, dotStyle: dot, hover: "", peek: false }));
    out.push({
      n: "5",
      style: Object.assign({}, base, { cursor: "default", color: "#1D1D1F", fontWeight: 600 }),
      emStyle: { fontStyle: "normal", display: "inline-grid", placeItems: "center", width: "26px", height: "26px", borderRadius: "8px", boxShadow: "inset 0 0 0 1.5px #2B47E0", fontSize: "12px", color: "#2B47E0" },
      dotStyle: { display: "block", width: "3px", height: "3px", borderRadius: "50%", background: "#2B47E0", marginTop: "3px", animation: "wPulse 1.6s infinite" },
      hover: "", peek: false, tap: null
    });
    [6, 7, 8].forEach(g => out.push({
      n: String(g),
      style: Object.assign({}, base, { cursor: "pointer" }),
      emStyle: { fontStyle: "normal", display: "inline-grid", placeItems: "center", width: "24px", height: "24px", border: "1px dashed #C0BDB6", borderRadius: "50%", fontSize: "11px", color: "#AEAEB2" },
      dotStyle: dot, hover: "background:#EEECE7", peek: false,
      tap: () => this.openDay(g, true)
    }));
    return out;
  }
  ...
  calendar() {
    ...
    if (!this.state.persomeConnected || !liveDays.length) return this.legacyCalendar();
    const now = new Date();
  ...
  renderVals() {
    ...
    const day = s.day != null && !s.shadow ? (s.persomeConnected ? liveDayMap[s.day] : DAYS[s.day]) : null;
    ...
    const dockSource = s.persomeConnected && Array.isArray(live.connectors) && live.connectors.length
      ? live.connectors
      : s.dock;
    ...
    const dayReading = day
      ? (day.selfReading || DAY_READINGS[s.day] || fallbackReading)
      : null;
    ...
    const identityDay = liveDays[0] || (s.modelSnapshot ? {
      key: "identity",
      short: "IDENTITY",
    ...
    const identityDayKey = identityDay?.key || 31;
    const identityReading = identityDay?.selfReading || DAY_READINGS[identityDayKey] || {
    ...
    return {
      ...
      setupNeedsProfile: !s.setupStatus?.profile,
      setupHasProfile: !!s.setupStatus?.profile,
      setupNeedsRuntime: !s.setupStatus?.ready,
      ...
      goSkyStop: e => { e.stopPropagation(); this.setState({ view: "sky", starRead: -1, skySel: -1, skyFocus: -1, evidenceFocus: null, menuOn: false, dockOpen: false }); },
      menuSky: () => this.setState({ menuOn: false, view: "sky", starRead: -1, skySel: -1, skyFocus: -1, evidenceFocus: null, dockOpen: false }),
      dropSky: () => this.setState({ view: "sky", starRead: -1, skySel: -1, skyFocus: -1, evidenceFocus: null, askDrop: false, dockOpen: false }),
      ...
      proBriefOn: !s.visitor && s.view === "home" && (this.props.proactiveStyle ?? "briefing") === "briefing",
      proPushOn: !s.visitor && s.view === "home" && (this.props.proactiveStyle ?? "briefing") === "push",
      proHeadline: "Now",
      proWhisper: "过去 · 现在 · 未来",
      proDateLabel: s.persomeConnected ? (live.proactiveLabel || "").replace(/\s*·\s*LIVE$/i, "") : "THU 8/6",
      proItems: s.persomeConnected && Array.isArray(live.nowItems) && live.nowItems.length
        ...
        : [
            { kind: "过去", marker: "↶", iconUrl: appIconUrl("Coast"), t: "昨天", title: "7 月 31 日，发生了什么", why: "回到一个差点被忘记的片段", rowStyle: { display: "grid", gridTemplateColumns: "34px 48px minmax(0,1fr) auto", gap: "10px", alignItems: "center", padding: "12px 2px", borderTop: "1px solid rgba(60,60,67,.10)", cursor: "pointer" }, tap: () => this.openDay(31, false) },
            { kind: "现在", marker: "—", iconUrl: appIconUrl("ChatGPT"), t: "现在", title: "继续 Personal Card", why: "从刚才留下的上下文开始", rowStyle: { display: "grid", gridTemplateColumns: "34px 48px minmax(0,1fr) auto", gap: "10px", alignItems: "center", padding: "12px 2px", borderTop: "1px solid rgba(60,60,67,.10)", cursor: "pointer" }, tap: () => this.openDay(31, false) },
            { kind: "未来", marker: "○", iconUrl: appIconUrl("Notes"), t: "明天", title: "明天可能再回到这里", why: "先替你整理上下文与未完成项", rowStyle: { display: "grid", gridTemplateColumns: "34px 48px minmax(0,1fr) auto", gap: "10px", alignItems: "center", padding: "12px 2px", borderTop: "1px solid rgba(60,60,67,.10)", cursor: "pointer", opacity: .76 }, tap: () => this.openDay(6, true) }
          ],
      proOpen: () => futureEvents.length ? this.openDay(futureEvents[0].day, false) : latestDayKey ? this.openDay(latestDayKey, false) : this.openDay(6, true),
      proCal: () => this.setState({ view: "month", rw: "cal" }),
      proFoot: "不是待办，只是时间留下的线索",
      proMoreT: "时间",
      proOpenT: "看看明天",
      spotlightPh: s.ph || "搜索你记得的事…",
      spotlightSky: () => this.setState({ view: "sky", starRead: -1, skySel: -1, skyFocus: -1, evidenceFocus: null }),
      spotlightMcp: () => this.openMcp(),
      proPushes: s.persomeConnected && Array.isArray(live.pushes) && live.pushes.length ? live.pushes.map(p => ({
      ...
      clockLabel: s.persomeConnected && live.clockLabel ? live.clockLabel : "as of now",
      cardMonthYear: s.persomeConnected ? ((live.monthLabel || "August").toUpperCase() + " / " + (live.yearLabel || "2026")) : "AUGUST / 2026",
      monthLabel: s.persomeConnected ? (live.monthLabel || "August") : "July",
      yearLabel: s.persomeConnected ? (live.yearLabel || "2026") : "2026",
      graphLabel: s.persomeConnected ? ((live.monthLabel || "August").toUpperCase() + " · " + ((live.themes || []).length || 0) + " live themes") : "JULY · 6 themes",
      ...
      heroLineOn: !s.visitor && s.view === "home" && !s.dockOpen,
      heroLineText: s.persomeConnected ? "今天 · Persome 记录了 " + ((liveDays[0] && liveDays[0].events || []).length || 0) + " 个实时活动段" : "连接自己的 Personal Model 后，这里会出现你的活动",
      ...
      identityUpdated: s.persomeConnected ? (live.clockLabel || "as of now") : "offline snapshot",
      identityOnline: s.persomeConnected ? "model online" : "model offline",
      ...
      starFresh: focusedEvidence ? "真实事件 · 可回溯" : s.starRead === 99 ? (s.persomeConnected ? "Persome · 实时读取" : "每晚重建 · 基于全部记忆") : s.starRead >= 0 ? liveFacts[s.starRead].f : "",
      starSource: e => { e.stopPropagation(); const requestedDay = focusedEvidence?.day || (s.starRead === 99 ? (latestDayKey || 31) : liveFacts[s.starRead].day); const d = liveDayMap[requestedDay] || !s.persomeConnected ? requestedDay : latestDayKey; this.setState({ flipped: false, starRead: -1, evidenceFocus: null }); if (d) this.openDay(d, false); },
      ...
      obsText: s.persomeConnected && live.observation ? live.observation : "观察：这周有三个晚上，你在 23 点后又打开了 spec —— 点开看那几天",
      obsGo: () => latestDayKey ? this.openDay(latestDayKey, false) : this.openDay(29, false),
      ...
      hardFacts: [
        { t: "判断先问「哪条路错了代价最小」——三次都选了可回滚的。", r: "face_012 · 验证 7/30" },
        { t: "偏好根因修复，拒绝过能立刻止血的 workaround。", r: "face_011 · PR #71" },
        { t: "写规范先把边界写死，再往里填功能。", r: "face_009 · 7/29" }
      ],
      wall: s.wall,
      ...
      onAsk: e => {
        ...
        if (!q) {
          ...
          if (s.persomeConnected && livePrompt) {
            this.setState({ ans: [livePrompt.title + "——" + livePrompt.why, "Persome · 最近活动 · ⏎ 下一条", livePrompt.day || null], ri: (s.ri + 1) % live.proactive.length });
            return;
          }
          const r = REM[s.ri];
          this.setState({ ans: [r[0], "[receipt:" + r[1] + "] · 主动提醒 · ⏎ 下一条", null], ri: (s.ri + 1) % REM.length });
          return;
        }
        e.target.value = "";
        if (s.visitor) {
          const a = this.answerFor(q);
          this.setState({ askDrop: false, ans: [a[0], "— public card · 已与私有 Persome 隔离", null] });
        }
        else if (s.persomeConnected) this.askPersome(q, false);
        else {
      ...
      dockSummary: s.persomeConnected ? "最近出现的应用" : "可连接的应用",
      dockSummarySub: s.persomeConnected ? "来自 Persome + Coast 的最近活动" : "Persome 离线，显示演示连接",
      ...
      spotlightSearch: e => {
        ...
        const pool = s.persomeConnected ? liveDays : [DAYS[31], DAYS[30], DAYS[29]].map((d, i) => Object.assign({ key: [31, 30, 29][i] }, d));
      ...
      rwTimes: s.persomeConnected ? liveDays.slice(0, 3).map((d, i) => ({ t: i === 0 ? "最近一天" : d.short, tap: () => this.openDay(d.key, false) })) : [["今天", 31], ["昨天", 30], ["上周", 29]].map(([t, n]) => ({ t, tap: () => this.openDay(n, false) })),
      rwApps: (s.persomeConnected ? (live.apps || []).map(a => [a.name, a.icon, a.color, "#FFFFFF", a.time, a.day]) : [["Cursor", "◤", "#E6E6E9", "#1D1D1F", "6h 35m", 31], ["Notion", "N", "#EFEDE7", "#3B3833", "5h 40m", 29], ["Figma", "F", "#F7DFE6", "#C2356B", "2h 13m", 29], ["Zoom", "Z", "#E2E7F3", "#3155A8", "2h 18m", 30], ["Arc", "A", "#DCECEF", "#137A8C", "2h 22m", 30], ["Mail", "✉", "#F5E4D7", "#C1562B", "1h 02m", 30]]).map(([t, ic, bg, fg, h, day]) => ({
      ...
      gNote: s.persomeConnected ? ((live.themes || []).length + " 个实时主题 · 来自最近 7 天的 Persome 活动；节点大小只表示当前注意力。") : "6 个主题 · 82 段 · 大小只反映投入时间，边只反映段落共现 —— 结论由你来下。",
      ...
      dayTotal: day && day.totalTime ? day.totalTime : "9h 12m",
      ...
      daySource: day && day.source ? day.source : "V5 historical replay",
      portrait: s.portraits[s.day] || (day ? day.portrait : ""),
      portraitReceipt: day ? (s.persomeConnected ? "Persome · 由当天 " + day.events.length + " 个实时活动段归纳" : "face · 由 " + day.title.slice(day.title.indexOf(",") + 2) + " 当天 " + day.events.length + " 段推出") : "",
      ...
      dayNarrLabel: s.persomeConnected ? "Persome 实时归纳 · " : "23:59，它写下的 · ",
      taught: day ? day.taught : "",
      evCount: day ? day.events.length : 0,
      dayFoot: s.shadow ? "shadow plan · generated from your last 14 days" : (s.persomeConnected ? (day && day.source ? day.source : "Persome · live") : "frames burn in 24h · the story is forever")
    };
  }
}
